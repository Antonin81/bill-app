...
describe("Given I am connected as an employee on NewBill Page", () => {
  ...
  describe("When I am completing the form", () => {
    ...
    test("Then i submit the form and it should be sent", async ()=>{
      const store = null
      window.onNavigate(ROUTES_PATH.NewBill)
      ...
      window.onNavigate.mockClear();
      await waitFor(()=> screen.getByText(/Mes notes de frais/))
      expect(screen.getByText(/Mes notes de frais/)).toBeTruthy();
    })

    test("creates a image path from an API and fails with 404 message error", async () => {
      ...
      expect(consoleSpy.mock.calls[0][0].toString()).toEqual("Error: Erreur 404");
      consoleSpy.mockClear()
    })
    test("creates a image path from an API and fails with 500 message error", async () => {
      const consoleSpy = jest.spyOn(console, "error");
      jest.spyOn(mockStore, "bills")
      mockStore.bills.mockImplementationOnce(() => {
        return {
      ...
      await new Promise(process.nextTick);
      expect(consoleSpy).toHaveBeenCalled();
      expect(consoleSpy.mock.calls[0][0].toString()).toEqual("Error: Erreur 500");
      consoleSpy.mockClear()
    })
